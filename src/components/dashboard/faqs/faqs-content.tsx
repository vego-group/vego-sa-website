...
import { AddFaqPopup } from "./add-faq-popup";
...
function FaqsContent() {
  ...
  return (
    <div className="w-full space-y-4 sm:space-y-6 lg:space-y-8 pb-8">
      ...
      <FaqsActions onNewFaq={() => setIsCreateFaqOpen(true)} />

      {/* Table Section */}
      <div className="bg-white/5 backdrop-blur-sm rounded-xl sm:rounded-2xl border border-white/10 overflow-hidden">
        <FaqsTabs activeTab={activeTab} onTabChange={setActiveTab} />
        <FaqsTable activeTab={activeTab} searchQuery={searchQuery} />
      </div>
