...
export default function ContactCards() {
  ...
  return (
    <section className="py-10" dir={isArabic ? "rtl" : "ltr"}>
      <div className="mx-auto grid max-w-7xl gap-6 px-6 sm:grid-cols-3">
        {cards.map((card, index) => (
          ...
          >
            <div className={`mb-4 flex items-center gap-3 flex-row`}>
            ...
            {card.link ? (
              ...
              <p
                className={`text-base text-slate-600 ${isArabic ? "text-right" : "text-left"}`}
              >
                {card.value}
              </p>
            )}
